...
function setTool(thickness, button) {
...
function setEmoji(emoji, button) {
  currentEmoji = emoji;
  thinButton.classList.remove("selectedTool");
  thickButton.classList.remove("selectedTool");
  document.querySelectorAll("button").forEach((btn) => btn.classList.remove("selectedTool"));
...
function createBrushLine(initialX, initialY, thickness) {
...
function createBrushPreview(x, y, thickness) {
  return {
    draw(ctx) {
      ctx.beginPath();
...
function createEmojiPreview(x, y, emoji) {
  return {
    draw(ctx) {
